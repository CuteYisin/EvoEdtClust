...
//considering Cauchy scale is 1.0, a`v scale? sigma should be ? 
//Q determined by node.n, to maintain sigma 0.1 ~ 10
double PStableLSH::getOptimalSigma() {
    targetFP = 0.1 / node.n;
    //targetFP = 1.0 / node.n / node.n;
...
void PStableLSH::work() {
    ...
    for(int iter = 1; iter <= T; ++ iter) {
        ...
        //std::cerr << "~~~ from iteration start to fingerprints use " << (double)(clock() - st) / CLOCKS_PER_SEC << std::endl;
        ...
        //std::cerr << "~~~ from iteration start to dsu merged use " << (double)(clock() - st) / CLOCKS_PER_SEC << std::endl;
    }
